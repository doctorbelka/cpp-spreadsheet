...
public:
    ...
    double Evaluate(std::function<double(Position)>& args) const override {
        switch (type_){
            ...
            case Divide :
                if (rhs_->Evaluate(args)==0) throw FormulaError(FormulaError::Category::Div0);
                else {
                    return lhs_->Evaluate(args)/rhs_->Evaluate(args);
                }
...
public:
    ...
    double Evaluate(std::function<double(Position)>& args) const override {
         switch (type_){
            case UnaryPlus : return operand_->Evaluate(args);
                
            case UnaryMinus : return -operand_->Evaluate(args);
                
            default :  throw std::invalid_argument("unidentified operation type");
        }
    }
